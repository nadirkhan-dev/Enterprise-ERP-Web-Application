...
export default defineEventHandler(async (event) => {
  ...
  try {
    const queryResult = await sdk.ok(sdk.run_inline_query({
      ...
      body: {
        model: 'connect',
        view: 'connect_item_quarterly_dt',
        fields: [
          'connect_item_quarterly_dt.order_quarter',
          'connect_item_quarterly_dt.order_dollars',
          'connect_item_quarterly_dt.order_units',
        ],
        filters: {
          'connect_item_quarterly_dt.item_code': String(itemCode),
        },
        sorts: ['connect_item_quarterly_dt.order_quarter desc'],
    ...
    for (const row of rawRows as any[]) {
      const label = formatQuarterLabel(row['connect_item_quarterly_dt.order_quarter'])
      if (!label) continue
      rowsByQuarter.set(label, {
        bookedSales: Number(row['connect_item_quarterly_dt.order_dollars'] || 0),
        orderCount: Number(row['connect_item_quarterly_dt.order_units'] || 0),
      })
    }
    ...
    console.error('Looker item quarterly chart query failed:', queryError)
