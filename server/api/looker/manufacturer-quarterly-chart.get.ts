...
export default defineEventHandler(async (event) => {
  const { manufacturerName } = getQuery(event)

  if (!manufacturerName) {
    throw createError({ statusCode: 400, statusMessage: 'manufacturerName query parameter is required' })
  }
  ...
  try {
    const queryResult = await sdk.ok(sdk.run_inline_query({
      ...
      body: {
        model: 'connect',
        view: 'connect_manufacturer_quarterly_dt',
        fields: [
          'connect_manufacturer_quarterly_dt.order_quarter',
          'connect_manufacturer_quarterly_dt.order_dollars',
          'connect_manufacturer_quarterly_dt.order_units',
        ],
        filters: {
          'connect_manufacturer_quarterly_dt.manufacturer_name': String(manufacturerName),
        },
        sorts: ['connect_manufacturer_quarterly_dt.order_quarter desc'],
    ...
    for (const row of rawRows as any[]) {
      const label = formatQuarterLabel(row['connect_manufacturer_quarterly_dt.order_quarter'])
      if (!label) continue
      rowsByQuarter.set(label, {
        bookedSales: Number(row['connect_manufacturer_quarterly_dt.order_dollars'] || 0),
        orderCount: Number(row['connect_manufacturer_quarterly_dt.order_units'] || 0),
      })
    }
    ...
    console.error('Looker manufacturer quarterly chart query failed:', queryError)
