...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const queryResult = await sdk.ok(sdk.run_inline_query({
      ...
      body: {
        model: 'connect',
        view: 'connect_customer_quarterly_dt',
        fields: [
          'connect_customer_quarterly_dt.doc_quarter',
          'connect_customer_quarterly_dt.order_count',
          'connect_customer_quarterly_dt.order_dollars',
        ],
        filters: {
          'connect_customer_quarterly_dt.account_number': String(accountNumber),
        },
        sorts: ['connect_customer_quarterly_dt.doc_quarter desc'],
    ...
    for (const row of rawRows as any[]) {
      const label = formatQuarterLabel(row['connect_customer_quarterly_dt.doc_quarter'])
      if (!label) continue
      rowsByQuarter.set(label, {
        bookedSales: Number(row['connect_customer_quarterly_dt.order_dollars'] || 0),
        orderCount: Number(row['connect_customer_quarterly_dt.order_count'] || 0),
      })
    }

    // Always emit the trailing N quarters (matches the dashboard's `Timeframe=9 quarter`).
    ...
    console.error('Looker quarterly chart query failed:', queryError)
